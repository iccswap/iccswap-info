...
function init() {
  ...
  try {
    const parsed = JSON.parse(window.localStorage.getItem(ICCSWAP))
...
export function Updater() {
  ...
  useEffect(() => {
    window.localStorage.setItem(ICCSWAP, JSON.stringify({ ...state, [LAST_SAVED]: Math.floor(Date.now() / 1000) }))
  })
