...
export function useHourlyRateData(pairAddress, timeWindow) {
  ...
  useEffect(() => {
    ...
    const windowSize = timeWindow === '1 day' ? 'day' : (timeWindow === timeframeOptions.MONTH ? 'month' : 'week')
    const startTime =
...
// 首页交易对列表
export function useHomeRateData(pairAddress) {
  ...
  useEffect(() => {
    ...
    async function fetch() {
      const chartData = await getHourlyRateData(pairAddress, startTime, latestBlock)
      if (chartData && !isUnmount) {
  ...
  }, [chartData, pairAddress, latestBlock,sub])
return chartData
}
...
/**
 * Get list of all pairs in Iccswap
 */
