...
import { useAllPairsInIccswap, useAllTokensInIccswap } from '../../contexts/GlobalData'
...
`
  ...
  let allTokens = useAllTokensInIccswap()
  const allTokenData = useAllTokenData()

  let allPairs = useAllPairsInIccswap()
  ...
  useEffect(() => {
    if (Object.keys(filteredTokenList).length > 5) {
      toggleShadow(true)
    } else {
      toggleShadow(false)
    }
  ...
  useEffect(() => {
    if (Object.keys(filteredPairList).length > 5) {
  ...
  const [tokensShown, setTokensShown] = useState(5)
    ...
    <Container small={small}>
      <Wrapper open={showMenu} shadow={true} small={small}>
        <Input
          ...
          type={'text'}
          ref={wrapperRef}
          placeholder={'输入或粘贴地址或名称以搜索代币或交易对'}
          value={value}
          onChange={e => {
      ...
      <Menu hide={!showMenu} ref={menuRef}>
        ...
        <div>
          {filteredPairList && Object.keys(filteredPairList).length === 0 && (
            <MenuItem>
              <TYPE.body style={{ color: '#000' }}>暂无数据</TYPE.body>
            </MenuItem>
          )}
          {filteredPairList &&
              ...
              return (
                <BasicLink to={'/pair/' + pair.id} key={pair.id} onClick={onDismiss}>
                  <MenuItem>
                    ...
                    <TYPE.body style={{ marginLeft: '10px', color: '#000' }}>
          ...
          <Heading
            hide={!(Object.keys(filteredPairList).length > 5 && Object.keys(filteredPairList).length >= pairsShown)}
          >
            <Blue
              onClick={() => {
        ...
        <div>
              ...
              <BasicLink to={'/token/' + token.id} key={token.id} onClick={onDismiss}>
                <MenuItem>
                  <RowFixed>
                    ...
                    <FormattedName text={token.name} maxCharacters={20} style={{ marginRight: '6px', color: '#000' }} />
                    (<FormattedName text={token.symbol} maxCharacters={6} style={{ color: '#000' }} />)
                  </RowFixed>
                </MenuItem>
              </BasicLink>
            )
          ...
          <Heading
            hide={!(Object.keys(filteredTokenList).length > 5 && Object.keys(filteredTokenList).length >= tokensShown)}
          >
            <Blue
              onClick={() => {
