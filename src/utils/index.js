...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://app.iccswap.com/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://app.iccswap.com/#/swap?inputCurrency=${
      token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token0Address
...
export const toNiceDate = date => {
  let x = dayjs.utc(dayjs.unix(date)).format('DD日')
  return x
}
...
export const toNiceDateYear = date => dayjs.utc(dayjs.unix(date)).format('YYYY-MM-DD')
